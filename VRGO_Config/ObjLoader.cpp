...
{
	{
		...
		pMesh->m_aNormalArray = new ObjNormal[(sizeof(ObjNormal)*pMesh->m_iVBO_Size)];
...
void DeleteMesh(ObjMesh* pMesh)
{
	...
	*/
	if(pMesh)
	{
		...
		if (pMesh->m_aBiNormalArray)
		{
			free(pMesh->m_aBiNormalArray);
			pMesh->m_aBiNormalArray = NULL;
		}
		...
		/*	delete the Colour array */
		if (pMesh->m_aColourArray)
		{
			free(pMesh->m_aColourArray);
			pMesh->m_aColourArray = NULL;
		}
